...
class PCD_Align(nn.Module):
    ...
    '''
    def __init__(self, nf=64, groups=8, opt=None):
    ...
    def forward(self, fea1, fea2, t=None, t_back=None):
        ...
        L3_offset = self.lrelu(self.L3_offset_conv2_1(L3_offset)) if t is None or 3 not in self.opt['level'] else self.lrelu(self.L3_offset_conv2_1(L3_offset)) + self.TMB_A_l3(L3_offset, t)
        ...
        L2_offset = self.lrelu(self.L2_offset_conv2_1(torch.cat([L2_offset, L3_offset * 2], dim=1))) # why there is 2
        L2_offset = self.lrelu(self.L2_offset_conv3_1(L2_offset)) if t is None or 2 not in self.opt['level'] else self.lrelu(self.L2_offset_conv3_1(L2_offset)) + self.TMB_A_l2(L2_offset, t)
        ...
        L1_offset = self.lrelu(self.L1_offset_conv2_1(torch.cat([L1_offset, L2_offset * 2], dim=1))) # why there is 2
        L1_offset = self.lrelu(self.L1_offset_conv3_1(L1_offset)) if t is None or 1 not in self.opt['level'] else self.lrelu(self.L1_offset_conv3_1(L1_offset)) + self.TMB_A_l1(L1_offset, t)
        ...
        L3_offset = self.lrelu(self.L3_offset_conv2_2(L3_offset)) if t_back is None or 3 not in self.opt['level'] else self.lrelu(self.L3_offset_conv2_2(L3_offset)) + self.TMB_B_l3(L3_offset, t_back)
        ...
        L2_offset = self.lrelu(self.L2_offset_conv3_2(L2_offset)) if t_back is None or 2 not in self.opt['level'] else self.lrelu(self.L2_offset_conv3_2(L2_offset)) + self.TMB_B_l2(L2_offset, t_back)
        ...
        L1_offset = self.lrelu(self.L1_offset_conv3_2(L1_offset)) if t_back is None or 1 not in self.opt['level'] else self.lrelu(self.L1_offset_conv3_2(L1_offset)) + self.TMB_B_l1(L1_offset, t_back)
...
class Network(nn.Module):
    def __init__(self, nf=64, nframes=3, groups=8, front_RBs=5, back_RBs=10, opt=None):
        ...
        self.fea_L3_conv1 = nn.Conv2d(nf, nf, 3, 2, 1, bias=True)
        self.fea_L3_conv2 = nn.Conv2d(nf, nf, 3, 1, 1, bias=True)
        self.pcd_align = PCD_Align(nf=nf, groups=groups, opt=opt)
        self.fusion = nn.Conv2d(2 * nf, nf, 1, 1, bias=True)
